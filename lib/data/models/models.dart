...
class HabitModel {
  ...
  double get monthlyCompletionRate {
    ...
    for (int i = 0; i < 30; i++) {
      ...
      total++;
      if (completionLog[_dateKey(d)] == true) done++;
    }
  ...
  factory HabitModel.fromMap(Map<dynamic, dynamic> map) => HabitModel(
        id: map['id'] ?? '',
        userId: map['userId'] ?? '',
        title: map['title'] ?? '',
        emoji: map['emoji'] ?? '⭐',
        color: map['color'] ?? '#7C6EF5',
        frequency: map['frequency'] ?? 'daily',
        completionLog: (map['completionLog'] as Map?)
